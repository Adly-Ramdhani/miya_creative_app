...
<body class="bg-background text-on-surface font-body-md min-h-screen flex flex-col">
    ...
    <main class="flex-1 flex flex-col min-h-screen">
        ...
        <div class="max-w-5xl mx-auto w-full px-6 py-lg md:px-lg pb-32 md:pb-lg">
            ...
            <form action="/admin/update/{{ $admin->id }}"  method="POST" enctype="multipart/form-data" class="flex flex-col gap-md">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-lg p-md rustic-border soft-shadow group">
                    ...
                    <div class="relative aspect-square rounded-lg border-2 border-dashed border-primary/30 bg-surface-container-low flex flex-col items-center justify-center overflow-hidden cursor-pointer hover:bg-surface-container transition-colors group-hover:border-primary/60">
                        ...
                        <!-- Preview -->
                        <img id="image-preview" src="{{ asset('uploads/' . $admin->photo) }}" class="absolute inset-0 w-full h-full object-cover">

                        <!-- Upload -->
                        <input class="absolute inset-0 opacity-0 cursor-pointer" id="photo-upload" type="file" name="photo">

                        <!-- Placeholder -->
                        <div id="upload-placeholder" class="hidden text-center p-md">
                            <span class="material-symbols-outlined text-primary text-5xl mb-md">
                                add_a_photo
                            </span>
                ...
                <div class="bg-white rounded-lg p-md rustic-border soft-shadow flex flex-col gap-md">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ $admin->title }}" placeholder="Vintage Meadow Bouquet" class="w-full bg-white border border-primary/30 rounded-lg px-md py-base">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="category" name="category" class="w-full bg-white border border-primary/30 rounded-lg px-md py-base">
                            ...
                            <option value="bouquet" {{ $admin->category == 'bouquet' ? 'selected' : '' }}>
                                Bouquet
                            </option>

                            <option value="cake" {{ $admin->category == 'cake' ? 'selected' : '' }}>
                                Cake
                            </option>

                            <option value="rustic" {{ $admin->category == 'rustic' ? 'selected' : '' }}>
                                Rustic
                            </option>

                            <option value="mahar" {{ $admin->category == 'mahar' ? 'selected' : '' }}>
                                Mahar
                            </option>

                            <option value="hampers" {{ $admin->category == 'hampers' ? 'selected' : '' }}>
                                Hampers
                            </option>
                    ...
                    <div>
                        ...
                        <input type="number" name="price" value="{{ $admin->price }}" placeholder="100000" class="w-full bg-white border border-primary/30 rounded-lg px-md py-base">
                    </div>
             ...
             <div>
                ...
                <div class="bg-white rounded-lg p-md rustic-border soft-shadow">
                    ...
                    <textarea name="description" rows="5" placeholder="Describe the product..." class="w-full bg-white border border-primary/30 rounded-lg px-md py-base resize-none">{{ $admin->description }}</textarea>
                </div>

                <!-- Submit -->
                <button id="save-btn" type="submit" class="px-md py-sm bg-primary text-on-primary rounded-lg flex items-center justify-center gap-2">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Ensure input accepts images
        if (uploadInput) uploadInput.setAttribute('accept', 'image/*');
        ...
        // Menu options map
        const menuOptions = {
            bouquet: ['flower bouquet','money bouquet','snack bouquet','doll bouquet'],
            cake: ['money cake','snack cake'],
            rustic: ['papan bunga rustic','hantaran rustic'],
            mahar: ['mahar'],
            hampers: ['hampers'],
            designundangan: ['undangan'],
            flowertable: ['flower table']
        };

        function populateMenu(category, selected) {
            if (!menuSelect) return;
            menuSelect.innerHTML = '<option value="">Select Menu</option>';
            if (category && menuOptions[category]) {
                menuOptions[category].forEach(m => {
        ...
        // initial populate with server values
        const initialCategory = "{{ $admin->category ?? '' }}";
        const initialMenu = "{{ $admin->menu ?? '' }}";
        populateMenu(initialCategory, initialMenu);

        if (categorySelect) {
            categorySelect.addEventListener('change', function () {
                populateMenu(this.value, null);
            });
        }
        ...
        // Micro-interaction for input glow
